...
export default (_config: any, app: Application) =>
    async (ctx: Context, next) => {
        const rootTraceId = Tracing.ejector.eject(ctx).getOrElse(() => Tracing.tracer.createRootId()); // get TraceId from Http Headers
        ...
        span.setLocalEndpoint(new Endpoint({ serviceName: ctx.application.name, port: ctx.protocol === 'http' ? 80 : 443 })); // maybe is arbitrary
